...
fd_callback(struct lws* wsi, enum lws_callback_reasons reason, MinnetCallback* cb, struct lws_pollargs* args) {
  ...
  static const char* io_events(int events) {
  ...
  static int io_parse_events(const char* str) {
    int events = 0;
...
#define PIO (POLLIN | POLLOUT)
  ...
  static JSValue minnet_io_handler(JSContext * ctx, JSValueConst this_val, int argc, JSValueConst* argv, int magic, JSValue* func_data) {
  ...
  static JSValue make_handler(JSContext * ctx, int fd, int events, void* opaque, int magic) {
    JSValue data[] = {
        JS_NewUint32(ctx, fd),
  ...
  void minnet_handlers(JSContext * ctx, struct lws * wsi, struct lws_pollargs * args, JSValue out[2]) {
    JSValue func;
  ...
  JSValue minnet_emit_this(const struct ws_callback* cb, JSValueConst this_obj, int argc, JSValue* argv) {
    if(!cb->ctx)
      return JS_UNDEFINED;
  ...
  JSValue minnet_emit(const struct ws_callback* cb, int argc, JSValue* argv) { return minnet_emit_this(cb, cb->this_obj /* ? *cb->this_obj : JS_NULL*/, argc, argv); }
  ...
  void value_dump(JSContext * ctx, const char* n, JSValueConst const* v) {
