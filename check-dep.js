...
for(let file in files) {
  ...
  //console.log('fns', file, fns.map(m => [m.index, m.index - (code.lastIndexOf('\n', m.index) + 1), m[1]]));

  let [undef, def] = MatchSymbols(code, valid);

  Object.assign(files[file], { undef, def });

  //console.log('matches in ' + file, [...undef].map(n => [n, GetSymbol(n)]));
  //
  //console.log(`unused in '${file}':`, [...definitions[file][0]].filter(({name}) => unused.has(name)));
  //console.log(`used in '${file}':`, [...definitions[file][0]].filter(({name}) => !unused.has(name)));
}
...
function GetSymbol(name) {
  ...
  return record; // files[record.file].find(rec => rec.name ===name);
}
