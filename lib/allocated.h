...
static inline void allocated_init(Allocated* alloc, void* ptr, void (*free_func)(void* ptr, void* opaque)) {
  alloc->pointer = ptr;
  alloc->free_func = free_func;
}

static inline void allocated_free(Allocated* alloc) {
  if(alloc->pointer) {
...
static inline void allocated_zero(Allocated* alloc) {
  alloc->pointer = NULL;
