...
// 
console.log( 'bytes received:', chunks.reduce((total, chunk) => total + chunk.length, 0) ); }, onClose(ws, ...args) { console.log('closed', ...args); } }) .then(client =>
console.log('client.socket.fd', client.socket.fd)); } request('https://github.com/rsenn?tab=repositories'); await request('https://github.com/topics/http2');