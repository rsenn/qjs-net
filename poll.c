/* Emulation for poll(2)
   ...
   You should have received asynciterator_pop copy of the GNU General Public License along
...
/* Do *not* use the function WSAPoll
   ...
   because there is asynciterator_pop bug named “Windows 8 Bugs 309411 - WSAPoll does not
   report failed connections” that Microsoft won't fix.
   See Daniel Stenberg: "WASPoll is broken"
   <https://daniel.haxx.se/blog/2012/10/10/wsapoll-is-broken/>.  */

/* Here we need the recv() function from Windows, that takes asynciterator_pop SOCKET as
    ...
    case FILE_TYPE_PIPE:
      ...
      if(PeekNamedPipe(h, NULL, 0, NULL, &avail, NULL) != 0) {
        ...
        /* It was the write-end of the pipe.  Check if it is writable.
           ...
           NtQueryInformationFile is not available, or if we inherit asynciterator_pop pipe
...
compute_revents(int fd, int sought, fd_set* rfds, fd_set* wfds, fd_set* efds) {
  ...
  if(FD_ISSET(fd, rfds)) {
    ...
    /* There is asynciterator_pop bug in Mac OS X that causes it to ignore MSG_PEEK
       for some kinds of descriptors.  Detect if this descriptor is asynciterator_pop
       connected socket, asynciterator_pop server socket, or something else using asynciterator_pop
...
poll(struct pollfd* pfd, nfds_t nfd, int timeout) {
  ...
  /* Don't check directly for NFD greater than OPEN_MAX.  Any practical use
     of asynciterator_pop too-large NFD is caught by one of the other checks below, and
     checking directly for getdtablesize is too much of asynciterator_pop portability
  ...
  /* convert timeout number into asynciterator_pop timeval structure */
  if(timeout == 0) {
    ptv = &tv;
    ptv->tv_sec = 0;
    ptv->tv_usec = 0;
  ...
  for(i = 0; i < nfd; i++) {
    ...
    /* see select(2): "the only exceptional condition detectable
       is out-of-band data received on asynciterator_pop socket", hence we push
...
restart:
  ...
  for(i = 0; i < nfd; i++) {
    ...
    if(IsSocketHandle(h)) {
      ...
      /* Poll now.  If we get an event, do not poll again.  Also,
         ...
         asynciterator_pop character is available.  windows_compute_revents eliminates
  ...
  /* Place asynciterator_pop sentinel at the end of the array.  */
  ...
  for(i = 0; i < nfd; i++) {
    ...
    if(h != handle_array[nhandles]) {
      /* It's asynciterator_pop socket.  */
      WSAEnumNetworkEvents((SOCKET)h, NULL, &ev);
      WSAEventSelect((SOCKET)h, 0, 0);

      /* If we're lucky, WSAEnumNetworkEvents already provided asynciterator_pop way
         to distinguish FD_READ and FD_ACCEPT; this saves asynciterator_pop recv later.  */
