macro(build_mbedtls)
  ...
  ExternalProject_Add(
    ...
    CMAKE_ARGS -DENABLE_TESTING=OFF -DENABLE_PROGRAMS=OFF "-DCMAKE_MODULE_PATH=${CMAKE_MODULE_PATH}" "-DCMAKE_SYSTEM_NAME=${CMAKE_SYSTEM_NAME}" "-DCMAKE_BUILD_TYPE=${MBEDTLS_BUILD_TYPE}"
    CMAKE_CACHE_ARGS
      "-DCMAKE_C_COMPILER:STRING=${CMAKE_C_COMPILER}" "-DCMAKE_C_COMPILER_ID:STRING=${CMAKE_C_COMPILER_ID}" "-DCMAKE_C_FLAGS:STRING=${MBEDTLS_C_FLAGS}" "-DCMAKE_C_FLAGS_DEBUG:STRING=${CMAKE_C_FLAGS_DEBUG}" "-DCMAKE_C_FLAGS_MINSIZEREL:STRING=${CMAKE_C_FLAGS_MINSIZEREL}" "-DCMAKE_C_FLAGS_RELEASE:STRING=${CMAKE_C_FLAGS_RELEASE}" "-DCMAKE_C_OUTPUT_EXTENSION:STRING=.o"
      "-DCMAKE_C_COMPILER_WORKS:BOOL=TRUE" "-DCMAKE_AR:STRING=${CMAKE_AR}" "-DCMAKE_C_CREATE_STATIC_LIBRARY:INTERNAL=${CMAKE_C_CREATE_STATIC_LIBRARY}" "-DCMAKE_C_LINK_EXECUTABLE:STRING=${CMAKE_C_LINK_EXECUTABLE}" "-DCMAKE_STATIC_LIBRARY_PREFIX_C:STRING=${CMAKE_STATIC_LIBRARY_PREFIX_C}" "-DCMAKE_STATIC_LIBRARY_PREFIX_CXX:STRING=${CMAKE_STATIC_LIBRARY_PREFIX_CXX}"
  ...
  add_custom_target(${MBEDTLS_TARGET_NAME}_clean COMMAND ${CMAKE_COMMAND} --build ${BINARY_DIR} --target clean WORKING_DIRECTORY "${BINARY_DIR}" COMMENT "Cleaning mbedtls" VERBATIM)
  ...
  set(MBEDTLS_LIBRARIES "${MBEDTLS_LIBRARY};${MBEDTLS_CRYPTO_LIBRARY};${MBEDTLS_X509_LIBRARY}" CACHE STRING "MbedTLS libraries" FORCE)
