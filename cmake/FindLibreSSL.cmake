macro(find_libressl)
  ...
  if(LIBRESSL_INCLUDE_DIR AND EXISTS
                              "${LIBRESSL_INCLUDE_DIR}/libressl/version.h")
    file(STRINGS "${LIBRESSL_INCLUDE_DIR}/libressl/version.h"
         VERSION_STRING_LINE
    ...
    string(REGEX
           REPLACE "^#define LIBRESSL_VERSION_STRING[ \\t\\n\\r]+\"([^\"]*)\"$"
                   "\\1" LIBRESSL_VERSION "${VERSION_STRING_LINE}")
    string(REGEX REPLACE "^#define LIBRESSL_VERSION_MAJOR[ \\t\\n\\r]+([0-9]+)$"
                         "\\1" LIBRESSL_VERSION_MAJOR "${VERSION_MAJOR_LINE}")
    string(REGEX REPLACE "^#define LIBRESSL_VERSION_MINOR[ \\t\\n\\r]+([0-9]+)$"
                         "\\1" LIBRESSL_VERSION_MINOR "${VERSION_MINOR_LINE}")
    string(REGEX REPLACE "^#define LIBRESSL_VERSION_PATCH[ \\t\\n\\r]+([0-9]+)$"
                         "\\1" LIBRESSL_VERSION_PATCH "${VERSION_PATCH_LINE}")
  endif()
  ...
  find_library(LIBRESSL_CRYPTO_LIBRARY NAMES ${_LIBRESSL_CRYPTO_LIB_NAME}
               PATH_SUFFIXES lib HINTS ${LIBRESSL_ROOT_DIR} ${_EXTRA_FIND_ARGS})
  find_library(LIBRESSL_SSL_LIBRARY NAMES ${_LIBRESSL_SSL_LIB_NAME}
               PATH_SUFFIXES lib HINTS ${LIBRESSL_ROOT_DIR} ${_EXTRA_FIND_ARGS})
  find_library(LIBRESSL_TLS_LIBRARY NAMES ${_LIBRESSL_TLS_LIB_NAME}
               PATH_SUFFIXES lib HINTS ${LIBRESSL_ROOT_DIR} ${_EXTRA_FIND_ARGS})

  set(LIBRESSL_LIBRARIES ${LIBRESSL_TLS_LIBRARY}^${LIBRESSL_SSL_LIBRARY}
  ...
  find_package_handle_standard_args(
    ...
    REQUIRED_VARS
      LIBRESSL_INCLUDE_DIR LIBRESSL_SSL_LIBRARY LIBRESSL_CRYPTO_LIBRARY
  ...
  if(LIBRESSL_FOUND)
    ...
    set(LIBRESSL_LIBRARY_DIR "${LIBRARY_DIR}"
        CACHE PATH "LibreSSL library directory")
    set(LIBRESSL_INCLUDE_DIR "${LIBRESSL_INCLUDE_DIR}"
        CACHE PATH "LibreSSL include directory")
