...
string(REPLACE " " "\n\t" ARGS "${LIBWEBSOCKETS_ARGS}")
  ...
  ExternalProject_Add(
    ...
    CMAKE_ARGS -DLWS_WITH_SHARED:BOOL=OFF -DLWS_WITH_STATIC:BOOL=ON -DLWS_STATIC_PIC:BOOL=ON "-DCMAKE_C_COMPILER:FILEPATH=${CMAKE_C_COMPILER}" "-DCMAKE_C_FLAGS:STRING=${LIBWEBSOCKETS_C_FLAGS}" "-DCMAKE_VERBOSE_MAKEFILE:BOOL=${CMAKE_VERBOSE_MAKEFILE}" "-DCMAKE_INSTALL_RPATH:STRING=${MBEDTLS_LIBRARY_DIR}" "-DCOMPILER_IS_CLANG:BOOL=OFF" "-DCMAKE_BUILD_TYPE:STRING=${CMAKE_BUILD_TYPE}"
               "-DCMAKE_LIBRARY_PATH:PATH=${CMAKE_LIBRARY_PATH};${CMAKE_CURRENT_BINARY_DIR}/mbedtls;${CMAKE_CURRENT_BINARY_DIR}/brotli" ${LIBWEBSOCKETS_ARGS}
    CMAKE_CACHE_ARGS
      -DDISABLE_WERROR:BOOL=ON -DLWS_HAVE_LIBCAP:BOOL=FALSE -DLWS_ROLE_RAW_PROXY:BOOL=ON -DLWS_UNIX_SOCK:BOOL=ON -DLWS_WITH_ACCESS_LOG:BOOL=ON -DLWS_WITH_CGI:BOOL=OFF -DLWS_WITH_DISKCACHE:BOOL=ON -DLWS_WITH_DIR:BOOL=OFF -DLWS_WITH_EVLIB_PLUGINS:BOOL=OFF -DLWS_WITH_EXTERNAL_POLL:BOOL=ON -DLWS_WITH_FILE_OPS:BOOL=ON -DLWS_WITH_FSMOUNT:BOOL=ON -DLWS_WITH_HTTP2:BOOL=ON -DLWS_WITH_HTTP_BROTLI:BOOL=ON
      -DLWS_WITH_HTTP_PROXY:BOOL=ON -DLWS_WITH_HTTP_STREAM_COMPRESSION:BOOL=ON -DLWS_WITH_LEJP:BOOL=OFF -DLWS_WITH_LEJP_CONF:BOOL=OFF -DLWS_WITH_LIBUV:BOOL=OFF -DLWS_WITH_MINIMAL_EXAMPLES:BOOL=OFF -DLWS_WITH_NO_LOGS:BOOL=OFF -DLWS_WITHOUT_EXTENSIONS:BOOL=OFF -DLWS_WITHOUT_TESTAPPS:BOOL=ON -DLWS_WITH_PLUGINS_API:BOOL=${LWS_WITH_PLUGINS_API} -DLWS_WITH_PLUGINS:BOOL=${LWS_WITH_PLUGINS}
      -DLWS_WITH_PLUGINS_BUILTIN:BOOL=${LWS_WITH_PLUGINS_BUILTIN} -DLWS_WITH_RANGES:BOOL=ON -DLWS_WITH_SERVER:BOOL=ON -DLWS_WITH_SOCKS5:BOOL=ON -DLWS_WITH_SYS_ASYNC_DNS:BOOL=ON -DLWS_WITH_THREADPOOL:BOOL=ON -DLWS_WITH_UNIX_SOCK:BOOL=ON -DLWS_WITH_ZIP_FOPS:BOOL=ON -DLWS_WITH_ZLIB:BOOL=ON
    INSTALL_COMMAND ""
    LOG_DOWNLOAD ON
   LOG_CONFIGURE ON
    LOG_BUILD ON)
